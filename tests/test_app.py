...
@pytest.fixture
def app_instance():
    """Provides a SpotifyApp instance for tests."""
    mock_integration = MagicMock()
    mock_integration.get_credentials.return_value = {"access_token": "dummy_access_token"}

    return SpotifyApp(integration=mock_integration)

def test_universal_mcp_spotify_app_initialization(app_instance):
    """
    Test that the SpotifyApp instance is initialized correctly with a name.
    """
    ...
    assert app_instance.name == "spotify", "SpotifyApp instance has unexpected name."


def test_universal_mcp_spotify_tool_docstrings_format(app_instance):
    """
    Test that each tool method in SpotifyApp has a well-formatted docstring,
...
def test_universal_mcp_spotify_tools_are_callable(app_instance):
    """
    Test that each tool method returned by list_tools in SpotifyApp is callable.
    """
    tools = app_instance.list_tools()
    assert isinstance(tools, list), "list_tools() should return a list."
